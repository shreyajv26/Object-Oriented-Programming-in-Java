...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
			...
			map = new UnfoldingMap(this, 200, 50, 650, 650, new Microsoft.RoadProvider());
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			boolean val = isInCountry(earthquake,m);
			if(val) {
				return true;
			}
		}
	...
	private void printQuakes() 
	{
		...
		for(Marker cm : countryMarkers) {
			...
			if (countryQuakesCount>0) {
				System.out.println("Country: " + cm.getProperty("name") + ", quakes count: " + countryQuakesCount);
			}
		}
		for (Marker quake : quakeMarkers) {
